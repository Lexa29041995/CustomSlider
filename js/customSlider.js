...
customSliderArr.forEach( item => item.customSlider = function(sliderSettings ={}) {
    ...
    outerContainer.style.width = `${slideWidth * maxSlides}px`;

    if(maxSlides >= 2 && margin > 0) {
        outerContainer.style.width = `(${slideWidth} * ${maxSlides}) + ${margin}px`;
    ...
    function autoPlay() {
        ...
        setTimeout(() => { clearInterval(timerStop)},timeout*(slides.length-1));
        }   
    };
    ...
    function buildSlider(slider, slideWidth, slideHeight, slides, navs, dots, margin) {
        slides.forEach(item => {
